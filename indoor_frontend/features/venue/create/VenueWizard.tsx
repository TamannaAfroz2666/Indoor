...
function Amenities({ errors, clearError }: StepProps) {
  ...
  function toggle(group: keyof typeof amenityGroups, item: string) { const values = draft.amenities[group]; const removing = values.includes(item); updateSection("amenities", { ...draft.amenities, [group]: removing ? values.filter((value) => value !== item) : [...values, item] }); if (group === "courtTypes" && removing) updateSection("spaces", draft.spaces.filter((space) => space.sport !== item)); clearError(group); }
  function setSpace(index: number, key: "name" | "sport" | "hourlyRate", value: string) { updateSection("spaces", draft.spaces.map((space, position) => position === index ? { ...space, [key]: value } : space)); }
  return <><Heading number={4} title="Amenities & Bookable Spaces" subtitle="Select amenities, then add each real court or space with its hourly rate." /><div className="grid gap-5 md:grid-cols-2">{(Object.keys(amenityGroups) as Array<keyof typeof amenityGroups>).map((group) => <fieldset key={group} className="rounded-xl border border-[#dfe7e3] p-5"><legend className="px-1 font-bold capitalize">{group === "courtTypes" ? "Court Types" : group === "facilities" ? "Sports & Facilities" : group === "environment" ? "Indoor / Outdoor" : "Additional Highlights"}</legend><div className="mt-2 grid gap-2 sm:grid-cols-2">{amenityGroups[group].map((item) => { const selected = draft.amenities[group].includes(item); return <button key={item} type="button" aria-pressed={selected} onClick={() => toggle(group, item)} className={`flex items-center justify-between rounded-lg border px-3.5 py-3 text-left text-sm transition ${selected ? "border-[#15b866] bg-[#eaf9f1] font-semibold text-[#087d47]" : "border-[#e1e8e4] hover:border-[#9fd7ba]"}`}><span>{item}</span><span className={`flex h-5 w-5 items-center justify-center rounded-full border ${selected ? "border-[#12b866] bg-[#12b866] text-white" : "border-[#b9c5bf]"}`}>{selected && <Check size={13} />}</span></button>; })}</div>{errors[group] && <p className="mt-3 text-xs text-red-600">{errors[group]}</p>}</fieldset>)}</div><section className="mt-6 rounded-xl border border-[#dfe7e3] p-5"><div className="flex items-center justify-between gap-4"><div><h3 className="font-bold">Bookable courts / spaces</h3><p className="mt-1 text-sm text-[#65756d]">Set the genuine BDT hourly rate for each space.</p></div><button type="button" disabled={!draft.amenities.courtTypes.length} onClick={() => updateSection("spaces", [...draft.spaces, { name: "", sport: draft.amenities.courtTypes[0] ?? "", hourlyRate: "" }])} className="rounded-lg bg-[#08ad59] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Add space</button></div><div className="mt-4 space-y-3">{draft.spaces.map((space, index) => <div key={index} className="grid gap-3 rounded-lg bg-[#f6f9f7] p-3 sm:grid-cols-[1.4fr_1fr_1fr_auto]"><input aria-label={`Space ${index + 1} name`} value={space.name} onChange={(e) => setSpace(index, "name", e.target.value)} className={inputClass} placeholder="Indoor Court 1" /><select aria-label={`Space ${index + 1} sport`} value={space.sport} onChange={(e) => setSpace(index, "sport", e.target.value)} className={inputClass}>{draft.amenities.courtTypes.map((sport) => <option key={sport}>{sport}</option>)}</select><input aria-label={`Space ${index + 1} hourly rate`} type="number" min="1" step="1" value={space.hourlyRate} onChange={(e) => setSpace(index, "hourlyRate", e.target.value)} className={inputClass} placeholder="BDT / hour" /><button type="button" onClick={() => updateSection("spaces", draft.spaces.filter((_, position) => position !== index))} className="mt-2 self-center rounded-lg p-3 text-red-600"><X size={18} /></button>{[errors[`space-${index}-name`], errors[`space-${index}-sport`], errors[`space-${index}-rate`]].filter(Boolean).map((error) => <p key={error} className="text-xs text-red-600 sm:col-span-4">{error}</p>)}</div>)}</div>{errors.spaces && <p className="mt-3 text-xs text-red-600">{errors.spaces}</p>}</section></>;
}
