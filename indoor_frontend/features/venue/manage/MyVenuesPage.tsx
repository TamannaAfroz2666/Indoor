...
function VenueRow({ venue }: { venue: ApiVenue }) {
  ...
  return (
    <article className="border-b border-[#e5ebe8] p-4 last:border-b-0 sm:p-5 lg:flex lg:gap-6">
      <div className="relative h-44 w-full shrink-0 overflow-hidden rounded-lg bg-[#edf1ef] sm:h-48 lg:h-[150px] lg:w-[230px]"><Image unoptimized src={venueThumbnail(venue)} alt={venue.name} fill sizes="(max-width: 1024px) 100vw, 230px" className="object-cover" /><span className="absolute bottom-2 left-2 inline-flex items-center gap-1 rounded-full bg-black/70 px-2.5 py-1 text-xs text-white"><ImageIcon size={14} /> {venue._count?.photos ?? venue.photos?.length ?? 0}</span></div>
