...
export function OwnerBookingRequestsPage({ venueId }: { venueId: string }) {
  ...
  async function transitionBooking(booking: OwnerBooking, nextStatus: "ACCEPTED" | "DECLINED") {
    if (nextStatus === "DECLINED" && !window.confirm("Decline this booking request?")) return;
    setTransitioningId(booking.id);
    setActionError("");
    try {
      const result = nextStatus === "ACCEPTED" ? await bookingApi.accept(booking.id) : await bookingApi.decline(booking.id);
      setBookings((current) => current.map((item) => item.id === booking.id ? { ...item, ...result.booking } : item));
    } catch (reason) {
      setActionError(reason instanceof Error ? reason.message : "Unable to update this booking request.");
    } finally {
    ...
    <main className="min-h-screen bg-[#f7f9f8] text-[#17251e]">
      <div className="mx-auto grid max-w-[1600px] lg:grid-cols-[310px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 p-4 sm:p-7 xl:p-9">
          ...
          {actionError && <div role="alert" className="mt-4 flex items-center justify-between gap-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700"><span>{actionError}</span><button type="button" onClick={() => setActionError("")} aria-label="Dismiss error"><X size={16} /></button></div>}
          ...
          {bookings.length === 0 ? <Empty title="No booking requests yet" detail="New requests for this venue will appear here." /> : filtered.length === 0 ? <Empty title="No matching requests" detail="Try changing the selected filters." /> : <>
            <div className="mt-4 hidden overflow-hidden rounded-xl border border-[#dfe7e3] bg-white xl:block"><div className="grid grid-cols-[1.35fr_1.2fr_.8fr_1.2fr_.7fr_.7fr_.65fr_1fr] gap-4 bg-[#fafbfa] px-5 py-4 text-xs font-semibold text-[#637169]"><span>Requester</span><span>Venue & Space</span><span>Sport</span><span>Date & Time</span><span>Players</span><span>Amount</span><span>Status</span><span>Action</span></div>{filtered.slice(0, visible).map((booking) => <DesktopRow key={booking.id} booking={booking} busy={transitioningId === booking.id} onTransition={transitionBooking} />)}</div>
            <div className="mt-4 space-y-3 xl:hidden">{filtered.slice(0, visible).map((booking) => <MobileCard key={booking.id} booking={booking} busy={transitioningId === booking.id} onTransition={transitionBooking} />)}</div>
...
type ActionProps = { booking: OwnerBooking; busy: boolean; onTransition: (booking: OwnerBooking, status: "ACCEPTED" | "DECLINED") => Promise<void> };
function Actions({ booking, busy, onTransition }: ActionProps) { return booking.status === "PENDING" ? <div className="flex gap-2"><button type="button" disabled={busy} onClick={() => void onTransition(booking, "DECLINED")} className="rounded-lg border border-red-300 px-3 py-2 text-xs font-semibold text-red-600 disabled:cursor-wait disabled:opacity-50">{busy ? "Updating…" : "Decline"}</button><button type="button" disabled={busy} onClick={() => void onTransition(booking, "ACCEPTED")} className="rounded-lg bg-[#08a958] px-3 py-2 text-xs font-semibold text-white disabled:cursor-wait disabled:opacity-50">{busy ? "Updating…" : "Accept"}</button></div> : <button type="button" onClick={() => alert(booking.message || "No message was included with this request.")} className="rounded-lg border border-[#a9dabe] px-3 py-2 text-xs font-semibold text-[#078d4a]">View Details</button>; }
function DesktopRow({ booking, busy, onTransition }: ActionProps) { return <article className="grid grid-cols-[1.35fr_1.2fr_.8fr_1.2fr_.7fr_.7fr_.65fr_1fr] items-center gap-4 border-t border-[#e5ebe8] px-5 py-5"><Requester booking={booking} /><div><p className="text-sm font-bold">{booking.venue.name}</p><p className="mt-1 text-xs text-[#607068]">{booking.space.name}</p><span className="mt-2 inline-block rounded-full bg-[#e5f8ed] px-2 py-0.5 text-[10px] text-[#078647]">{booking.venue.venueType}</span></div><p className="text-sm">{booking.space.sport}</p><div className="text-sm"><p className="font-semibold">{dateOnly.format(new Date(booking.startAt))}</p><p className="mt-2 text-xs">{timeOnly.format(new Date(booking.startAt))} – {timeOnly.format(endAt(booking))}</p></div><div className="text-sm"><p className="flex items-center gap-1"><Users size={14} /> {booking.participants ?? "—"}</p><p className="mt-2 text-xs text-[#607068]">{booking.duration} mins</p></div><p className="text-sm font-bold">{currency.format(booking.estimatedRate)}</p><Status status={booking.status} /><Actions booking={booking} busy={busy} onTransition={onTransition} /></article>; }
function MobileCard({ booking, busy, onTransition }: ActionProps) { return <article className="rounded-xl border border-[#dfe7e3] bg-white p-4"><div className="flex items-start justify-between gap-3"><Requester booking={booking} /><Status status={booking.status} /></div><div className="mt-4 grid grid-cols-2 gap-4 border-y border-[#edf1ef] py-4 text-sm"><div><p className="text-xs text-[#728078]">Venue & space</p><p className="mt-1 font-semibold">{booking.venue.name}</p><p className="text-xs">{booking.space.name} · {booking.space.sport}</p></div><div><p className="text-xs text-[#728078]">Date & time</p><p className="mt-1 font-semibold">{dateOnly.format(new Date(booking.startAt))}</p><p className="text-xs">{timeOnly.format(new Date(booking.startAt))} – {timeOnly.format(endAt(booking))}</p></div><div><p className="text-xs text-[#728078]">Players / duration</p><p className="mt-1 font-semibold">{booking.participants ?? "—"} / {booking.duration} mins</p></div><div><p className="text-xs text-[#728078]">Amount</p><p className="mt-1 font-bold">{currency.format(booking.estimatedRate)}</p></div></div><div className="mt-4 flex justify-end"><Actions booking={booking} busy={busy} onTransition={onTransition} /></div></article>; }
